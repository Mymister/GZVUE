<template>
  <acticle>
    ...
    <ul class="mui-table-view">
		...
		<li v-for="item in photoCategoryList" v-bind:key="item.id" class="mui-table-view-cell" >{{item.title}}</li>
	</ul>
...
export default {
  ...
  methods:{
      getPhotoCategoryList(){
          this.axios.get(this.api.getPC)
          .then(rep => this.photoCategoryList =rep.data.message);
      },
      // 获取图片列表, 需要使用分类ID来获取指定的图片列表
      getPhotoList() {
            this.axios.get(this.api.getPL + this.$route.params.id)
